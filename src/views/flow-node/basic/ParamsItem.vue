...
<template>
  <div class="params-item-wrapper" v-if="titles && titles.length">
    <div
      :class="[item.flexNum ? 'flex-' + item.flexNum : 'p-i-col-' + (i + 1)]"
      v-for="(item, i) in props.titles"
      :key="i"
    >
  ...
  <div :key="showPageIndex">
    <div
      ...
      v-for="(item, i) in paramsData"
      :key="item.id"
    >
      <div
        :class="[
          props.titles[j].flexNum
            ? 'flex-' + props.titles[j].flexNum
            : 'p-i-col-' + (j + 1)
        ]"
        v-for="(v, j) in item.list"
        :key="j"
      >
        <div class="p-i-content" :key="'content' + j">
          <div
            v-if="v.type === 'input'"
            class="p-i-content-item"
            :class="[
          ...
          <div v-if="v.type === 'input-text'" class="p-i-content-item">
            <div class="p-i-input-text" :style="v.style">
          ...
          <div v-if="v.type === 'delete-icon'" class="p-i-content-item">
            <IconifyIconOffline
              class="p-i-w-delete-icon"
              @click="deleteItem(item, i, v)"
              v-if="!v.disabled"
              :style="v.style"
              :icon="IconDelete"
            />
  ...
  <div class="params-item-footer" v-if="props.showAddIcon">
    <el-button class="p-i-f-button" @click="addItem">新增</el-button>
  </div>
</template>
