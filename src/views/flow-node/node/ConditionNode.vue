...
import ArrowDown from "../flowIcons/ArrowDown.vue";
import RightIcon from "../flowIcons/RightIcon.vue";
import DragIcon from "../flowIcons/DragIcon.vue";
import DeleteIcon from "../flowIcons/DeleteIcon.vue";
...
import { computed, defineEmits, markRaw, ref, watch, onMounted } from "vue";
...
const deleteItem = (params: any, colIndex: number) => {
  ...
  const len =
    conditionNode.value.draggableListData[colIndex]?.paramsData?.length;
  if (len === 0) {
    // len 为 0 同时删除当前分支
...
<template>
  <div ref="modeRef" class="cond-model-node-wrapper">
    <div :class="[openCard ? 'cond-model-node-header' : '']">
      <div class="cond-model-node-title">
        ...
        />
        <img
          src="../images/icon-condition.png"
          alt="cm-n-t-start-icon"
          class="cm-n-t-start-icon"
        />
        <div class="cm-n-t-text">
          <el-input
            v-model="conditionNode.title.value"
            :placeholder="conditionNode.title.placeholder"
            :disabled="conditionNode.title.disabled"
      ...
      <div class="cond-model-node-subtitle" v-if="openCard">
        <span
          >连接多个下游分支，若设定的条件成立则仅运行对应的分支，若均不成立则只运行“否则”分支</span
        >
      </div>
    </div>
    <div class="cond-model-node-branch-title" v-if="openCard">
      ...
      <div class="c-m-n-title-right" @click="addBranch">
        <el-button class="c-m-n-add-icon" :icon="AddIcon" />
        <span class="c-m-n-r-text">新增分支</span>
      </div>
    </div>
    <div class="cond-model-node-content mt12" @mouseleave="dragMouseleave">
      <DraggableList
      ...
      >
        <template #default="{ data, index }: { data: any; index: number }">
          <div class="cm-n-drag-title">
            ...
            <div
              class="cm-n-c-label"
              v-if="computedShowLabel(data.inputTitles)"
            >
              {{ data.label }}{{ index + 1 }}
            </div>
            <div class="cm-n-c-right" v-if="data.inputTitles !== null">
              <el-icon
                class="cm-n-c-delete-icon"
                :class="[
                  computedDisabledBranchDelIcon(index)
                    ? 'cm-n-c-delete-icon-disabled'
                    : ''
                ]"
...
div[data-id^="10007-"]
  div[data-handlepos="right"]:not(div[data-handleid^="target-"]) {
  display: none;
}
