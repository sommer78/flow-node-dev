...
const addItem = () => {
  ...
  const id =
    startNode.value.inputParams.length === 0
      ? 1
      : startNode.value.inputParams[startNode.value.inputParams.length - 1].id +
        1;
...
<template>
  <div
    ref="modeRef"
    class="start-node-wrapper custom-node transition-colors duration-300"
    :class="[
  ...
  >
    <div :class="[openCard ? 'start-node-header' : '']">
      <div class="start-node-title">
        ...
        />
        <img
          src="/src/assets/images/icon-start.png"
          alt="s-n-t-start-icon"
          class="s-n-t-start-icon"
        />
        <span class="s-n-t-text">开始</span>
      </div>
      <div v-if="openCard" class="start-node-subtitle">
        <span>工作流的起始节点，用于设定启动工作流需要的信息</span>
      </div>
    </div>

    <div v-if="openCard" class="start-node-content">
      <el-collapse @change="collapseChange" v-model="openCollapseArr">
        <el-collapse-item name="input" :disabled="true">
          <template #title>
            <el-button
              class="s-n-collapse-icon"
          ...
          <div>
            <ParamsItem :data="startNode.inputParams" @addItem="addItem" />
          </div>
        </el-collapse-item>
      </el-collapse>
    </div>
