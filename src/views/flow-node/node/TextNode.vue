...
import RightIcon from "../flowIcons/RightIcon.vue";
import NoticeIcon from "../flowIcons/NoticeIcon.vue";
import AddIcon from "../flowIcons/AddIcon.vue";
import SettingIcon from "../flowIcons/SettingIcon.vue";
...
import TextStringToolTipCard from "../components/TextStringToolTipCard.vue";
import {
  toopTipText12,
  toopTipText13,
  toopTipText14,
...
import {
  ...
  checkBoxParam,
  deleteIconParam
} from "@/data/node-params-template";
...
const debounceUpdateNodeData = debounce((type: string) => {
  ...
  setTimeout(() => {
    ...
    const data = flowStore.getNodeDataById(nodeId.value);
...
const addItemInput = () => {
  ...
  const id =
    textNode.value.input.inputParams.length === 0
      ? 1
      : (
          textNode.value.input.inputParams[
            textNode.value.input.inputParams.length - 1
          ] as any
        )?.id + 1;
...
const useItem = (inputText: string) => {
  ...
  textNode.value.splitString.options.splice(
    textNode.value.splitString.options.length - 1,
    0,
    {
      label: inputText,
      value: inputText,
...
const delSelectItem = (value: string, index: number) => {
  textNode.value.splitString.options =
    textNode.value.splitString.options.filter(
  ...
  setTimeout(() => {
    textNode.value.splitString.value = textNode.value.splitString.value.filter(
      (item: any) => item !== value
    );
  }, 80);
};
...
const keepClose = (key: string) => {
  ...
  const findIndex = openCollapseArr.value.findIndex(
...
const showSettingStringArea = ref(false);
const settingStringAreaRef: any = ref<HTMLElement>();
const settingString = () => {
  setTimeout(() => {
    showSettingStringArea.value = true;
  }, 10);
};
...
const closeOrOpenSettingStringArea = (e: any) => {
  if (!settingStringAreaRef.value?.contains(e.target)) {
    showSettingStringArea.value = false;
  }
};
onMounted(() => {
...
<template>
  <div
    ref="modeRef"
    class="text-node-wrapper transition-colors duration-300"
    :class="[
  ...
  >
    <div :class="[openCard ? 'text-node-header' : '']">
      <div class="text-node-title">
        ...
        <img
          ...
          alt="t-n-t-start-icon"
          class="t-n-t-start-icon"
        />
        <div class="t-n-t-text">
          <el-input
            v-model="textNode.title.value"
            :placeholder="textNode.title.placeholder"
            :disabled="textNode.title.disabled"
    ...
    <div class="text-node-select-app" v-if="openCard">
      ...
      <div class="t-n-s-a-select">
        <el-select
          v-model="textNode.select.value"
          :placeholder="textNode.select.placeholder"
          :disabled="textNode.select.disabled"
    ...
    <div class="text-node-content" v-if="openCard">
      <el-collapse @change="collapseChange" v-model="openCollapseArr">
        <div class="text-node-content-item">
          <el-collapse-item name="input" :disabled="true">
            <template #title>
              <el-button
                class="t-n-collapse-icon"
            ...
            <div>
              <ParamsItem
                :data="textNode.input.inputParams"
                key="bmInput"
                :titles="inputTitles"
          ...
          <el-collapse-item name="string" :disabled="true">
            <template #title>
              ...
              >
                <template #reference>
                  <el-icon
                    class="t-n-c-header-icon t-n-c-header-icon-export-margin"
                  >
                    <NoticeIcon />
                  </el-icon>
                </template>
                <div class="t-n-c-text-table">
                  <TextStringToolTipCard />
                </div>
              </el-popover>
              <el-tooltip
              ...
              <div
                class="t-n-c-header-right"
                v-if="textNode.select.value === '1'"
              >
                <el-button
                  class="t-n-c-header-add-icon"
                  :icon="SettingIcon"
                  @click="settingString"
                />
                <div
                  class="t-n-c-setting-form"
                  v-if="showSettingStringArea"
                  ref="settingStringAreaRef"
                >
                  <div class="t-n-c-setting-form-title">数组连接符设置</div>
                  <div class="t-n-c-setting-form-desc">
                    使用以下符号来自动连接数组中的每个项目
                  </div>
                  <div>
                    <div class="t-n-c-setting-form-subtitle">
                      <el-tooltip
                        :offset="12"
                        popper-class="tooltip-box-item-in-node"
                        effect="dark"
                      ...
                      <el-tooltip
                        ...
                        popper-class="tooltip-box-item-in-node"
                        effect="dark"
                        :content="toopTipText15"
                        placement="top"
                      >
                        <el-icon class="t-n-c-header-icon ml4">
                          <NoticeIcon />
                        </el-icon>
                      </el-tooltip>
                      <span class="t-n-c-setting-form-subtitle-required"
                        >*</span
                      >
                    </div>
                    <div class="t-n-c-setting-form-content">111</div>
                  </div>
                </div>
              </div>
            </template>
            <div class="t-n-c-result-content">
              ...
              >
                ...
                >
                  ...
                  />
                  <div
                    class="t-n-c-select-option"
                    v-else-if="oItem.type === 'custom'"
                  >
                    <el-checkbox
                      v-model="oItem.checkBoxOptions.selected"
                  ...
                  <div class="t-n-c-select-option" v-else>
                    <el-checkbox
                      v-model="oItem.checkBoxOptions.selected"
                      :disabled="oItem.checkBoxOptions.disabled"
                      :style="oItem.checkBoxOptions.style"
...
.t-n-c-setting-form-content {
  margin-top: 4px;
  line-height: 1;
}
.t-n-c-setting-form-subtitle {
  font-size: 14px;
  line-height: 14px;
  font-weight: 400;
  color: rgb(56, 55, 67);
